...
class InternVLChatModel(PreTrainedModel):
    ...
    def forward(
        ...
        # vit_pooled = vit_embeds.mean(dim=1) # (B, C)
        # vit_pooled = cls_embeds.squeeze(1) # (B, C)
        
        # h = w = int(self.num_image_token ** 0.5)  # Calculate h and w from num_image_token
        # for i in range(seg_pos.shape[0]):
        #     if seg_pos[i].sum() > 0:
        #         # vit_pooled_bs = vit_pooled[big_patch_indices[i]:big_patch_indices[i+1]].mean(dim=0) # (C)
        #         # input_embeds[i, seg_pos[i]] = input_embeds[i, seg_pos[i]] + vit_pooled_bs.unsqueeze(0).repeat(seg_pos[i].sum(), 1) + self.seg_img_embed.weight.repeat(seg_pos[i].sum(), 1)
        #         # input_embeds[i, seg_pos[i]] = vit_pooled_bs.unsqueeze(0).repeat(seg_pos[i].sum(), 1) + self.seg_img_embed.weight.repeat(seg_pos[i].sum(), 1)
        #         input_embeds[i, seg_pos[i]] = input_embeds[i, seg_pos[i]] + self.seg_img_embed.weight.repeat(seg_pos[i].sum(), 1)
        #         input_embeds[i, img_pos[i]] = input_embeds[i, img_pos[i]] + self.seg_img_embed.weight.repeat(img_pos[i].sum(), 1)
        #         # input_embeds[i, img_pos[i]] = input_embeds[i, img_pos[i]] + input_embeds[i, seg_pos[i]][0].unsqueeze(0)
        #         # input_embeds[i, img_pos[i]] = input_embeds[i, img_pos[i]] + self.combine_position_embedding.repeat(img_pos[i].sum()//self.num_image_token, 1, 1).flatten(0, 1) + self.seg_img_embed.weight.repeat(img_pos[i].sum(), 1)
        #         ori_vit_embeds[big_patch_indices[i]:big_patch_indices[i+1]] = ori_vit_embeds[big_patch_indices[i]:big_patch_indices[i+1]] + self.seg_img_embed.weight.unsqueeze(0).repeat(big_patch_indices[i+1]-big_patch_indices[i], 1, 1)
        #         # ori_vit_embeds[big_patch_indices[i]:big_patch_indices[i+1]] = ori_vit_embeds[big_patch_indices[i]:big_patch_indices[i+1]] + input_embeds[i, seg_pos[i]][0].unsqueeze(0)

                # Apply positional encoding to the corresponding image patches
                # input_embeds[i, img_pos[i]] = self.apply_2d_sincos_pos_embed(input_embeds[i, img_pos[i]].view(
                #     big_patch_indices[i+1]-big_patch_indices[i], self.num_image_token, -1
                # ), h, w).flatten(0, 1)
                # ori_vit_embeds[big_patch_indices[i]:big_patch_indices[i+1]] = self.apply_2d_sincos_pos_embed(ori_vit_embeds[big_patch_indices[i]:big_patch_indices[i+1]], h*2, w*2)
    ...
    def conv2x2_shuffle(self, x):
        ...
        x_unfolded = x_padded.unfold(2, 2, 1)  # (B, C, H, W+1, 2)
        x_unfolded = x_unfolded.unfold(3, 2, 1)  # (B, C, H, W, 2, 2)
        x_unfolded = x_unfolded.contiguous().view(b, c, h, w, 4)  # (B, C, H, W, 4)
